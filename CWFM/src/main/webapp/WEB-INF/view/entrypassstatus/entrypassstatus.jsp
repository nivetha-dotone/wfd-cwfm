...
<div class="page-header">
   <!--  <form id="searchForm">
        ...
        <div>
   <label for="transactionId" style=" color: darkcyan;">TransactionID:</label>
         
    <c:if test="${empty GatePassObj.transactionId }">
    	<input id="transactionId" name="transactionId" style="width: 50%;height: 20px;" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="12" oninput="this.value = this.value.replace(/[^0-9]/g, '')" size="30" maxlength="12" autocomplete="off">
    </c:if>
     <c:if test="${not empty GatePassObj.transactionId }">
    	<input id="transactionId" name="transactionId" style="width: 50%;height: 20px;" type="text" size="30" maxlength="12" value="${GatePassObj.transactionId }" autocomplete="off" inputmode="numeric" pattern="[0-9]*" maxlength="12" oninput="this.value = this.value.replace(/[^0-9]/g, '')">
    </c:if>
    	
<input type="hidden" id="transactionId" name="transactionId">

<label for="gatePassId" style=" color: darkcyan;">GatepassNumber:</label>
        <c:if test="${empty GatePassObj.aadhaarNumber }">
    	<input id="gatePassId" name="gatePassId" style="width: 50%;height: 20px;" type="text"  size="30" maxlength="12" autocomplete="off">
    </c:if>
     <c:if test="${not empty GatePassObj.aadhaarNumber }">
    	<input id="gatePassId" name="gatePassId" style="width: 50%;height: 20px;" type="text" size="30" maxlength="12" value="${GatePassObj.gatePassId }" autocomplete="off" inputmode="numeric" pattern="[0-9]*" maxlength="12" oninput="this.value = this.value.replace(/[^0-9]/g, '')">
    </c:if>
<input type="hidden" id="gatePassId" name="gatePassId">
        <button type="button" class="btn btn-default process-footer-button-cancel ng-binding"  onclick="searchGatePassStatus()">Search</button>
