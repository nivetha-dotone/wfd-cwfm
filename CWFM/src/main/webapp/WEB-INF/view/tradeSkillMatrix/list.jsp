...
<head>
    ...
    <title>Trade Skill Mapping List</title>
...
<select id="deptId" name="deptId" style="color:gray;padding:3px;">
    <option value="">Select Department</option>
        ...
        <button type="button" class="btn btn-default process-footer-button-cancel ng-binding"  onclick="searchGatePassIdBasedOnPE()">Search</button>
 <!-- Inline script placed immediately after dropdowns -->

  </div>
    <div>
    <c:if test="${UserPermission.addRights eq 1 }">
         <button type="submit" class="btn btn-default process-footer-button-cancel ng-binding" onclick="redirectToTradeAdd()">Add</button> 
     </c:if>
     <c:if test="${UserPermission.editRights eq 1 }">
         <button type="submit" class="btn btn-default process-footer-button-cancel ng-binding" onclick="redirectToTradeEdit()">Edit</button> 
     </c:if> 
     <c:if test="${UserPermission.viewRights eq 1 }">
        <button type="submit" class="btn btn-default process-footer-button-cancel ng-binding" onclick="redirectToTradeView()">View</button>
...
<tr >
         ...
         <c:if test="${TradeSkillListingDto.size()>0 }">
        <tbody>
            <c:forEach items="${TradeSkillListingDto}" var="wo" >
            <tr>
