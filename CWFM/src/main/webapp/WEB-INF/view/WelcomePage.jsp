...
<script>
let lastSeenId = null; // Keep track of the last seen employee ID
...
function loadMoreEmployees() {
    ...
    $.ajax({
        type: "GET",
        url: "/CWFM/principalEmployer/list",
        data: {
            lastSeenId: lastSeenId, // Pass the last seen ID
            pageSize: pageSize
        },
        success: function(response) {
            // Append the new employees to the list
            $('#employeeList').append(response);
            
            // Update the last seen ID
            lastSeenId = getLastEmployeeId();
        },
...
function getLastEmployeeId() {
    // Get the ID of the last employee in the list
    return $('#employeeList').children().last().data('employee-id');
}
...
$(document).ready(function() {
 ...
 // Function to change page size
   function changePage() {
        var pageSize = 10; // Set default page size or retrieve it dynamically
        var selectedPage = document.getElementById("pageSelect").value;
        $.ajax({
            type: "GET",
            url: "/CWFM/principalEmployer/list?page=" + selectedPage + "&pageSize=" + pageSize,
            success: function(response) {
...
<body>
    ...
    <ul class="sub-menu">
       ...
       <!--  <li><a href="#" onclick="loadMinimumwageList(contextPath)">Minimum Wage Master</a></li> -->
         <li id="contractWorkmenMenuItem"><a href="#" onclick="loadCommonList(contextPath,'/contractworkmen/contractWorkmenList')">Contract Workmen</a></li>
