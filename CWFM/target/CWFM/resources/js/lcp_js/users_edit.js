...
function hyphen_generate(evt, value) {
    ...
    if (value.length === 3 && charCode != 8)
        document.getElementById('edit_users_phone_id').value = value.concat("-");
...
function validation() {
    var firstname = document.getElementById('edit_compres_fname_id').value;
    firstname = $.trim(firstname);
    var lastname = document.getElementById('edit_compres_lname_id').value;
    lastname = $.trim(lastname);
    var uniq_name = document.getElementById('edit_users_uniquename_id').value;
    uniq_name = $.trim(uniq_name);
    var addressline1 = document.getElementById('edit_users_addressline1_id').value;
    addressline1 = $.trim(addressline1);
    var state = document.getElementById('edit_users_state_id').value;
    var city = document.getElementById('edit_users_city_id').value;
    city = $.trim(city);
    var zip = document.getElementById('edit_users_zip_id').value;
    zip = $.trim(zip);
    var phone = document.getElementById('edit_users_phone_id').value;
    phone = $.trim(phone);
    var emailformat = /^[a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+[\.]{1}[a-zA-Z]{2,4}$/;
    var email = document.getElementById('edit_users_email_id').value;
    email = $.trim(email);
    var altemail = document.getElementById('edit_users_altemail_id').value;
    altemail = $.trim(altemail);
    var ssoid = document.getElementById('edit_users_ssoid_id').value;
    ssoid = $.trim(ssoid);
    var uniq_sso = document.getElementById('user_uniquessoid_id').value;
    uniq_sso = $.trim(uniq_sso);
    var useraccount = document.getElementById('edit_users_account_id').value;
    useraccount = $.trim(useraccount);
    var uniq_acc = document.getElementById('user_uniquness_name_id').value;
    uniq_acc = $.trim(uniq_acc);
    var password = document.getElementById('edit_users_password_id').value;
    password = $.trim(password);
    var confirmpassword = document.getElementById('edit_users_confirmpassword_id').value;
...
function addUniqueness(div_Id, tag_Id, tag_Name) {
    var old_fname = document.getElementById('edit_users_oldfname_id').value;
    old_fname = $.trim(old_fname);
    var old_lname = document.getElementById('edit_users_oldlname_id').value;
    old_fname = $.trim(old_fname);
    var f_name = document.getElementById('edit_compres_fname_id').value;
    f_name = $.trim(f_name);
    var l_name = document.getElementById('edit_compres_lname_id').value;
    l_name = $.trim(l_name);
    var first_name = document.getElementById('edit_compres_fname_id').value.split("#").join("ha_sh_sm-bl").split("^").join("%5E").split("?").join("qu_stn_mrk_sm-bl").split("/").join("fr_wrd_sl_sh_sm-bl").split("<").join("%3C").split(">").join("%3E").split("[").join("%5B").split("]").join("%5D").split("{").join("%7B").split("}").join("%7D").split("`").join("%60").split(".").join("dt_sm-bl").split("(").join("opn_br_ckt_sm-bl").split(")").join("cl_sd_br_ckt_sm-bl").split("$").join("dlr_sm-bl");
    first_name = $.trim(first_name);
    var last_name = document.getElementById('edit_compres_lname_id').value.split("#").join("ha_sh_sm-bl").split("^").join("%5E").split("?").join("qu_stn_mrk_sm-bl").split("/").join("fr_wrd_sl_sh_sm-bl").split("<").join("%3C").split(">").join("%3E").split("[").join("%5B").split("]").join("%5D").split("{").join("%7B").split("}").join("%7D").split("`").join("%60").split(".").join("dt_sm-bl").split("(").join("opn_br_ckt_sm-bl").split(")").join("cl_sd_br_ckt_sm-bl").split("$").join("dlr_sm-bl");
    last_name = $.trim(last_name);
    if (last_name === "" || first_name === "") {
        first_name = "null";
        last_name = "null";
    }
    if (f_name !== old_fname || l_name !== old_lname) {
        if (base_url == '' || base_url == null)
            base_url = document.getElementById('base_url').value;
        ...
        if (base_url == '' || base_url == null)
            base_url = document.getElementById('base_url').value;
        var url = base_url + 'ctrl_users/add_uniqueness/' + tag_Id + '/' + tag_Name + '/null/null';
        ajax(url, div_Id);
    }
}
